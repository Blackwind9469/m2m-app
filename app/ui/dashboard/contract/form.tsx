...
const ContractForm = ({
  ...
  const handleUnload = useCallback(async () => {
    if (edit && contract?.sim_id && contract?.device_id) {
      await updateUsedStatus(contract.sim_id, contract.device_id, true);
    }
  }, [edit, contract]);
  ...
  const clientAction = async (values: z.infer<typeof FormSchema>) => {
    ...
    try {
      localStorage.setItem("lastFormSubmission", JSON.stringify(values));
